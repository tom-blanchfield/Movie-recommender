...
nmf_factors = st.sidebar.slider("NMF latent factors", 2, 300, 10)
random_seed = st.sidebar.number_input("Random seed", value=42, step=1)

st.sidebar.divider()
st.sidebar.subheader("NMF Mode")
...
def load_npz(path):
    ...
    return {
        "H": d["H"],
        "movie_ids": d["movie_ids"].astype(int)
    }
...
if use_pretrained:
    ...
    H = npz_data["H"]
    nmf_movie_ids = npz_data["movie_ids"]
...
if st.button("Run Evaluation"):
    ...
    for i, uid in enumerate(test_users):
        ...
        for m in test_ids:
            ...
            if use_pretrained:
                ...
                for tid in train_ids:
                    if tid in user_vec.index:
                        user_vec[tid] = user_movie_matrix.loc[uid, tid]
                ...
                preds_full = latent @ H + mean_val
                pred_nmf = dict(zip(nmf_movie_ids, preds_full)).get(m, base_pred)
            else:
        ...
        if rmse_a is not None: rmses_app.append(rmse_a)
        if rmse_b is not None: rmses_base.append(rmse_b)
        if rmse_n is not None: rmses_nmf.append(rmse_n)
    ...
    if rmses_app: st.write(f"App — mean RMSE: {np.mean(rmses_app):.4f}")
    if rmses_base: st.write(f"Baseline — mean RMSE: {np.mean(rmses_base):.4f}")
    if rmses_nmf: st.write(f"NMF — mean RMSE: {np.mean(rmses_nmf):.4f}")
